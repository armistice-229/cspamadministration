...
async function chargerEffectifs() {
    ...
    try {
        // Pas de paramètre "annee" ici : le backend utilise automatiquement
        // l'année scolaire active (configurée dans les paramètres admin).
        const res = await fetch("https://cspam-backend.onrender.com/api/dashboard/effectifs", {
            headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
        });
        const data = await res.json();
